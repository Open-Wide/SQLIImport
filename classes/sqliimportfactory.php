...
final class SQLIImportFactory
{
    ...
    private function setLoggedInUser()
    {
        ...
        if( !$robotUserID )
        { // If no user id provided, take AnonymousUserID as a default value
            $robotUserID = $anonymousUserID;
        } else if( !is_int( $robotUserID ) )
        { // Invalid value
    ...
    public function runImport( array $aImportItems )
    {
        ...
        for( $i = 0, $iMax = count( $aImportItems ); $i < $iMax; ++$i )
        {
            ...
            {
                OWScriptLogger::logError( $e->getMessage(), 'importfactory' );
    ...
    {
        OWScriptLogger::writeNotice( 'Now cleaning the import process', 'importfactory' );
        $this->token = null;
        ...
        if( is_array( $this->scheduledImports ) )
        {
            foreach( $this->scheduledImports as $scheduledImport )
            {
                ...
                {
                    OWScriptLogger::logError( __METHOD__ . '$scheduledImport is not an instance of SQLIScheduledImport !', 'importfactory' );
                    continue;
                }
    ...
    public static function checkExistingHandlers( array $aHandlers )
    {
        ...
        if( $aHandlers )
        { // If $aHandlers is empty, then all defined handlers will be processed
    ...
    {
        $siteINI = eZINI::instance();
        if( $this->viewCachingEnabledGlobal )
        { // Restore view cache (enable it if it was enabled before import)
            $siteINI->setVariable( 'ContentSettings', 'ViewCaching', 'enabled' );
        }

        if( !$this->delayedIndexingEnabledGlobal )
        { // Restore DelayedIndexing (disable it if it was disabled before import)
