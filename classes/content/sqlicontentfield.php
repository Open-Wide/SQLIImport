...
class SQLIContentField
{
    ...
    public function isModified()
    {
        ...
        if( !in_array( $datatype, $aSkipDatatypes ) )
        { // Process diff operation for non-skipped datatypes
            $aDiffHandlerParams = array( $this->data, $this->attribute );

            if( isset( $diffHandlerMap[$datatype] ) )
            { // A diff handler is defined, use it
                $isModified = call_user_func_array( array( $diffHandlerMap[$datatype], 'contentIsModified' ), $aDiffHandlerParams );
            } else
            { // No diff handler for this datatype, use default diff handler
    ...
    public function fromString( $data )
    {
        ...
        { // Already known as implementing it
            $fromStringImplemented = true;
        } else if( in_array( $datatype, self::$datatypesFromStringNotImpl ) )
        { // Already known as NOT implementing it
        ...
        } else
        {
            ...
            { // Caller class is the original eZDataType class => fromString() is not implemented
                self::$datatypesFromStringNotImpl[] = $datatype;
                $fromStringImplemented = false;
            } else
            { // Caller class is the datatype itself, so we consider that fromString() is implemented
    ...
    public static function convertAttributeToString( eZContentObjectAttribute $attribute )
    {
        ...
        } else if( in_array( $datatype, self::$datatypesToStringNotImpl ) )
        { // Already known as NOT implementing it
